...
export async function ensureIgnored(repoRoot: string): Promise<void> {
  const exclude = join(repoRoot, ".git", "info", "exclude");
  try {
    const old = await readFile(exclude, "utf8");
    if (!old.split(/\r?\n/).includes("/.pi-agents/")) await writeFile(exclude, `${old}${old.endsWith("\n") ? "" : "\n"}/.pi-agents/\n`);
  } catch { /* linked worktrees use a gitfile; callers may already ignore the directory */ }
}
