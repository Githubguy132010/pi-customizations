...
import { __testing as settle, runSettleWorkflow } from "../../extensions/shared/commands/settle";
...
describe("settle internals", () => {
  it("parses JSON and classifies checks", () => {
    expect(settle.parseJson('{"a":1}')).toEqual({a:1}); expect(settle.parseJson("x")).toBeUndefined();
    expect(settle.hasPendingChecks(pr())).toBe(false);
    for (const value of ["expected","pending","queued","in_progress","requested","waiting"]) {
      expect(settle.hasPendingChecks(pr({ statusCheckRollup: [{ status: value }] }))).toBe(true);
      expect(settle.hasPendingChecks(pr({ statusCheckRollup: [{ state: value }] }))).toBe(true);
    }
    expect(settle.hasPendingChecks(pr({ statusCheckRollup: [{ status: "complete", state: "success" }] }))).toBe(false);
  });

  it("summarizes every check category", () => {
    expect(settle.checkSummary(pr())).toBe("checks unavailable");
    ...
    expect(settle.checkSummary(pr({statusCheckRollup: checks}))).toBe("3 passing, 5 failing, 6 pending");
  });

  it("offers actions based on state and pending checks", () => {
    expect(settle.actionOptions(pr()).map((x:any)=>x.action)).toEqual(["merge","close"]);
    expect(settle.actionOptions(pr({statusCheckRollup:[{status:"PENDING"}]})).map((x:any)=>x.action)).toEqual(["merge","auto","close"]);
    expect(settle.actionOptions(pr({state:"MERGED"}))).toEqual([{label:"Clean up branches for merged PR",action:"cleanup"}]);
  });
  ...
  it("reads valid, invalid, and failed pull requests", async () => {
    for (const [response, expected] of [[result(JSON.stringify(pr())), "pr"], [result("bad"), "error"], [result("",1,"fail"), "error"]] as const) {
      const pi=createPi({exec:vi.fn().mockResolvedValue(response)}); const got=await settle.readPullRequest(pi,"/r","1");
  ...
  it("selects PRs in RPC mode and handles listing variants", async () => {
    ...
    expect(await settle.selectOpenPullRequests(pi,createContext(),"/r")).toHaveLength(1);
    ...
    let ctx=createContext(); expect(await settle.selectOpenPullRequests(pi,ctx,"/r")).toEqual([]); expect(ctx.ui.notify).toHaveBeenCalled();
    ...
    expect((await settle.selectOpenPullRequests(pi,ctx,"/r"))[0].number).toBe(2);
    ctx=createContext({ui:createUi({select:vi.fn().mockResolvedValue(undefined)})});
    expect(await settle.selectOpenPullRequests(pi,ctx,"/r")).toEqual([]);
  });
  ...
  it("drives the TUI checklist renderer and every input", async () => {
    ...
    expect(await settle.selectOpenPullRequests(createPi({exec}),ctx,"/r")).toHaveLength(1);
    ...
    ctx.ui.custom=cancel; expect(await settle.selectOpenPullRequests(createPi({exec}),ctx,"/r")).toEqual([]);
  });
  ...
  it("chooses configured, origin, sole, selected, and absent remotes", async () => {
    ...
    expect(await settle.chooseRemote(pi,createContext(),"/r","main")).toBe("upstream");
    for(const [names,want] of [[[],undefined],[["origin","up"],"origin"],[["solo"],"solo"]] as any) {
      ...
      expect(await settle.chooseRemote(pi,createContext(),"/r","main")).toBe(want);
    }
    pi=createPi({exec:dispatcher({"git config --get branch.main.remote":result(".\n"),"git remote -v":result(remoteOut(["a","b"]))})});
    expect(await settle.chooseRemote(pi,createContext({ui:createUi({select:vi.fn().mockResolvedValue("b (url)")})}),"/r","main")).toBe("b");
  });

  it("checks branch existence", async()=>{
    expect(await settle.branchExists(createPi({exec:vi.fn().mockResolvedValue(result())}),"/r","x")).toBe(true);
    expect(await settle.branchExists(createPi({exec:vi.fn().mockResolvedValue(result("",1))}),"/r","x")).toBe(false);
  });
});
...
describe("settle cleanup and action execution", () => {
  const baseRoutes = {
  ...
  it("cleans an existing local feature branch", async () => {
    const ctx=createContext(); expect(await settle.cleanupLocalBranch(createPi({exec:dispatcher(baseRoutes)}),ctx,"/r",pr())).toBe(true);
  ...
  it("handles local cleanup preconditions", async()=>{
    let ctx=createContext(); expect(await settle.cleanupLocalBranch(createPi(),ctx,"/r",pr({headRefName:"main"}))).toBe(false);
    ctx=createContext(); expect(await settle.cleanupLocalBranch(createPi({exec:dispatcher({"git show-ref --verify --quiet refs/heads/feat":result("",1)})}),ctx,"/r",pr())).toBe(true);
    for(const routes of [
    ...
    ]) { ctx=createContext(); expect(await settle.cleanupLocalBranch(createPi({exec:dispatcher(routes)}),ctx,"/r",pr())).toBe(routes["git pull --ff-only origin main"]?.code===1); }
  });
  it("creates a missing base branch and skips checkout when already on it",async()=>{
    ...
    expect(await settle.cleanupLocalBranch(createPi({exec:dispatcher(routes)}),createContext(),"/r",pr())).toBe(true);
  ...
  const action = async (p:any, selections:any[], confirmations:any[], routes:any={}, dry=false) => {
    ...
    const ctx=createContext({ui}); const ok=await settle.settlePullRequest(createPi({exec:dispatcher(routes)}),ctx,"/r",p,dry); return {ok,ctx};
  };
...
describe("final settle branch cases",()=>{
 it("handles absent check arrays and values",()=>{
  expect(settle.hasPendingChecks(pr({statusCheckRollup:undefined}))).toBe(false);
  expect(settle.hasPendingChecks(pr({statusCheckRollup:[{}]}))).toBe(false);
  expect(settle.checkSummary(pr({statusCheckRollup:undefined}))).toBe("checks unavailable");
  expect(settle.checkSummary(pr({statusCheckRollup:[{}]}))).toBe("0 passing, 0 failing, 0 pending");
 });
 it("handles malformed lists, current empty lists, and current RPC selection",async()=>{
  ...
  let pi=createPi({exec:dispatcher({[view]:result(JSON.stringify(pr())),[list]:result("bad")})}); expect(await settle.selectOpenPullRequests(pi,createContext(),"/r")).toHaveLength(1);
  pi=createPi({exec:dispatcher({[view]:result(JSON.stringify(pr())),[list]:result(JSON.stringify([pr(),pr({number:2})]))})}); expect(await settle.selectOpenPullRequests(pi,createContext(),"/r")).toHaveLength(1);
 });
 it("covers TUI without a current PR, short rendering, empty enter, and sorting",async()=>{
  ...
  expect(await settle.selectOpenPullRequests(createPi({exec}),createContext({mode:"tui",ui:createUi({custom})}),"/r")).toHaveLength(2);
 });
 it("covers auto merge, absent refresh, draft close, and successful local cleanup",async()=>{
  ...
  await settle.settlePullRequest(createPi({exec:dispatcher(routes)}),createContext({ui}),"/r",pr({statusCheckRollup:[{status:"PENDING"}]}),false);
  ...
  await settle.settlePullRequest(createPi({exec:dispatcher(routes)}),createContext({ui}),"/r",pr({isDraft:true}),false);
  ...
  const ctx=createContext({ui});await settle.settlePullRequest(createPi({exec:dispatcher(routes)}),ctx,"/r",pr({state:"CLOSED"}),false);
 ...
 it("breaks a multi-PR workflow when an action is canceled",async()=>{
  ...
  await runSettleWorkflow("",createPi({exec}),createContext({ui}));expect(ui.select).toHaveBeenCalledTimes(2);
 });
});
